const BASE_URL = process.env.LOCAL_API_URL!;
...
const infoApi = {
  creatHeaders() {
    ...
    if (process.env.WORDPRESS_AUTH_PASSWORD) {
      headers["Authorization"] = 'Basic ' + Buffer.from(`${process.env.USERNAME}:${process.env.WORDPRESS_AUTH_PASSWORD}`).toString('base64') ;
    }
...
export async function updateFetchAPI(path:string = '', id:string = '', data: Record<string, any> = {}) {
